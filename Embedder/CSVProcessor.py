...
class CSVProcessor():
    ...
    async def extract_and_store_text_content(self):
        ...
        self.raw_text_content = df[self.configs["text_column"]].iloc[start_from:end_on].tolist()
        print(f"Completed text extraction. Total rows processed: {len(self.raw_text_content)}")
